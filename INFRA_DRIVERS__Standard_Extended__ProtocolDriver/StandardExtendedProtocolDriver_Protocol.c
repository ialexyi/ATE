...
typedef struct
{
	...
	tsSTD_Extended_CommandItem	*pSTD_CommandList;
	int							listSize;
...
//==============================================================================
// Static functions
void*   DLLEXPORT       STD_Driver_Send_Patern( int hCommunicationHandle , tsSTD_Extended_CommandItem tCommandItem , unsigned char *pData , int iSize );
...
void	DLLEXPORT	STD_Driver_Receive_CallBack( unsigned char *pReadBuffer , unsigned int iBufferLen , void *pData )
{
	STD_ERROR						StdError									=	{0};
	...
	IF (( pHandle == NULL ) , "Pointer to handle can't be NULL." );
	IF (( pReadBuffer == NULL ) , "Receive NULL pointer buffer." );   
	
	pBuffer = (unsigned short*)pReadBuffer;
	
	handle = *pHandle;
	
	if ( CmtGetTSVPtr ( handle , &pLocalStorage ) == 0 )
	{
		FREE_CALLOC_COPY( pLocalStorage->pSourceBuffer , iBufferLen + 100 , sizeof(char) , pReadBuffer , iBufferLen );   
		pLocalStorage->iSourceBufferLength = iBufferLen;
		
		pLocalStorage->iBufferLength = 0;
		GET_STDERR_CLEAR( pLocalStorage->pLastError );
	
		IF (( iBufferLen < ( MIN_MESSAGE_LEN + 2 )) , "Receive Wrong Packet");
		IF (( pBuffer[0] != pLocalStorage->pSTD_CommandList[iLastSentCommandIndex].responseHeader ) , "Receive Wrong Header");
	
		opCode = pBuffer[1]; 
		dataLength = pBuffer[2] + ( pBuffer[3] << 16 );
		
		IF (( iBufferLen < ( dataLength + MIN_MESSAGE_LEN + 2 )), "Receive Wrong Packet"); 
	
		for ( index = 0 ; index < (dataLength+MIN_MESSAGE_LEN) ; index++ )
			calcCheckSum += pReadBuffer[index];
	
		checkSum = *((short*)(&(pReadBuffer[index])));
		
		if ( dataLength < 512 )
		{
			IF (( checkSum != calcCheckSum ) , "Receive Wrong CheckSum");
		}
		
		FREE_CALLOC_ERR( pLocalStorage->pBuffer , dataLength + 100 , sizeof(char));
	
		for ( index = 0 ; index < dataLength ; index++ )  
			pLocalStorage->pBuffer[index] = pReadBuffer[index+MIN_MESSAGE_LEN];	
...
void*   DLLEXPORT       STD_Driver_Send_Patern( int hCommunicationHandle , tsSTD_Extended_CommandItem tCommandItem , unsigned char *pData , int iSize )
{
	...
	pBuffer[iSendLenth++] = tCommandItem.sendHeader;
	pBuffer[iSendLenth++] = tCommandItem.opCode;
	pBuffer[iSendLenth++] = (short)( iSize & 0x0FFFF );
	pBuffer[iSendLenth++] = (short)( iSize >> 16 );
	
	for ( index = 0 ; index < MIN_MESSAGE_LEN ; index++ )
		checkSumCalc += pSendBuffer[index];
	
	for ( index = 0 ; index < iSize ; index++)
		checkSumCalc += pSendBuffer[index+MIN_MESSAGE_LEN] = pData[index];

	iSendLenth = ( index + MIN_MESSAGE_LEN ) / 2;
	
	pBuffer[iSendLenth++] = (short)checkSumCalc;
...
void*   DLLEXPORT   STD_Driver_LoadConfigFile( int hHandle , char *pFileName )
{
	...
	FREE_CALLOC_ERR( pLocalStorage->pSTD_CommandList , 2048 , sizeof(tsSTD_Extended_CommandItem));
	
	for ( iIndex = 0 ; iIndex < 2048; iIndex++ )
	{
		iCount = ReadFile ( hFileHandle , pLocalStorage->pSTD_CommandList[iIndex].max_size , sizeof(tsSTD_Extended_CommandItem) );
		
		if ( iCount != sizeof(tsSTD_Extended_CommandItem) )
			break;
	}
...
void*   DLLEXPORT   STD_Driver_LoadConfigBuffer( int hHandle , char *pBuffer , int iBufferSize )
{
	...
	iCount = iBufferSize;
	iBaseSize = sizeof(tsSTD_Extended_CommandItem); 
