...
#include "Agilent_66XXA_wrapper.h"
...
void*	DLLEXPORT		Equipment_Info ( int hLowLevelHandle , char *pAddress , char **pVendor , char **pSerialNumber , char **pModel , char **pFirmware )
{
	...
	do
	{
		...
		IF (( strstr( szIdentificationUpperText , "HEWLETT-PACKARD" ) == NULL ) , "Is not supported" );
...
void*	DLLEXPORT		Equipment_IsSupported ( int hLowLevelHandle , char *pAddress , char *pID_String , char *pIdentificationText , int *pbSupporting , void **pLocalData )
{
	...
	char							vszSupportdedModels[][32]					=	{"6673A"};
	...
	do
	{
		...
		if ( strstr( szIdentificationUpperText , "HEWLETT-PACKARD" ) == NULL )
			break;	
...
void* DLLEXPORT PowerSupply_Init ( int hParentInstrumentHandle , ViRsrc szRsrcAddress , int *phInstrumentHandle , int *InstrumentConnectStatus , int *InstrumentChanelsQuantity )
{
	...
	// SYST:CHAN:COUN? not supported in 6673A
	if ( strstr ( pLocalHandle->pModel , "6673A" ))
	{
		iChannelsQuantity = 1;
	}
	
	else if ( viPrintf ( pLocalHandle->sessionHandle , "SYST:CHAN:COUN?\n" ) == 0 )
	{
...
//	IF ((( channel < pLocalHandle->iStartFromChannel ) || ( channel >= ( pLocalHandle->iStartFromChannel + pLocalHandle->iChannelsQuantity ))) , "Wrong channel number" );
	...
	for ( iTryIndex = 0; iTryIndex < 5; iTryIndex++ )
	{
		...
		strcpy(szReadFeedback, "VOLT?");
		CHK_VSA ( viWrite( pLocalHandle->sessionHandle , szReadFeedback, strlen(szReadFeedback), &count)); 
...
//	IF ((( channel < pLocalHandle->iStartFromChannel ) || ( channel >= ( pLocalHandle->iStartFromChannel + pLocalHandle->iChannelsQuantity ))) , "Wrong channel number" );
	...
	if ( plfVoltage )
	{
		strcpy(szReadFeedback, "MEAS:VOLT?");
		CHK_VSA ( viWrite( pLocalHandle->sessionHandle , szReadFeedback, strlen(szReadFeedback), &count)); 
		CHK_VSA ( viRead ( pLocalHandle->sessionHandle , szReadFeedback , LOW_STRING , &count ));
...
//	IF ((( channel < pLocalHandle->iStartFromChannel ) || ( channel >= ( pLocalHandle->iStartFromChannel + pLocalHandle->iChannelsQuantity ))) , "Wrong channel number" );
	...
	if ( plfCurrent )
	{
		strcpy(szReadFeedback, "MEAS:CURR?");
		CHK_VSA ( viWrite( pLocalHandle->sessionHandle , szReadFeedback, strlen(szReadFeedback), &count)); 
		CHK_VSA ( viRead ( pLocalHandle->sessionHandle , szReadFeedback , LOW_STRING , &count ));
...
//	IF ((( channel < pLocalHandle->iStartFromChannel ) || ( channel >= ( pLocalHandle->iStartFromChannel + pLocalHandle->iChannelsQuantity ))) , "Wrong channel number" );
	...
	for ( iTryIndex = 0; iTryIndex < 5; iTryIndex++ )
	{
		sprintf( szCommandString , "VOLT:LEV %E" , lfVoltage );
		CHK_VSA ( viPrintf( pLocalHandle->sessionHandle , szCommandString ));
	
		WaitForOperationCompletion( pLocalHandle->sessionHandle , pLocalHandle->lfTimeout , pLocalHandle->lfOpcLowLevelTimeout ); 
	
		strcpy(szReadFeedback, "VOLT:LEV?");
		CHK_VSA ( viWrite( pLocalHandle->sessionHandle , szReadFeedback, strlen(szReadFeedback), &count)); 
		CHK_VSA ( viRead ( pLocalHandle->sessionHandle , szReadFeedback , LOW_STRING , &count ));
...
//	IF ((( channel < pLocalHandle->iStartFromChannel ) || ( channel >= ( pLocalHandle->iStartFromChannel + pLocalHandle->iChannelsQuantity ))) , "Wrong channel number" );
	...
	for ( iTryIndex = 0; iTryIndex < 5; iTryIndex++ )
	{
		sprintf( szCommandString , "CURR:LEV %E" , lfCurrent);
		CHK_VSA ( viPrintf( pLocalHandle->sessionHandle , szCommandString ));
	
		WaitForOperationCompletion( pLocalHandle->sessionHandle , pLocalHandle->lfTimeout , pLocalHandle->lfOpcLowLevelTimeout ); 

		strcpy(szReadFeedback, "CURR:LEV?");
		CHK_VSA ( viWrite( pLocalHandle->sessionHandle , szReadFeedback, strlen(szReadFeedback), &count)); 
		CHK_VSA ( viRead ( pLocalHandle->sessionHandle , szReadFeedback , LOW_STRING , &count ));
...
//	IF ((( channel < pLocalHandle->iStartFromChannel ) || ( channel >= ( pLocalHandle->iStartFromChannel + pLocalHandle->iChannelsQuantity ))) , "Wrong channel number" );
	...
	if ( piState )
	{
		strcpy(szReadFeedback, "OUTP:STAT?");
		CHK_VSA ( viWrite( pLocalHandle->sessionHandle , szReadFeedback, strlen(szReadFeedback), &count)); 
		CHK_VSA ( viRead ( pLocalHandle->sessionHandle , szReadFeedback , LOW_STRING , &count ));
...
int		WaitForOperationCompletion( int sessionHandle , double lfTimeout , double lfLowLevelTimeout )
{
	...
	do
	{
		strcpy(szReadFeedback, "*OPC?\n");
		viWrite( sessionHandle , szReadFeedback, strlen(szReadFeedback), &count); 
...
void main ( void )
{
	...
	PowerSupply_Init ( 0 , "visa://192.168.145.149/GPIB0::7::INSTR" , &hInstrumentHandle , &InstrumentConnectStatus , &InstrumentChanelsQuantity );
	...
	PowerSupply_SetOutputVoltage ( hInstrumentHandle , 1 , 5.1 );
	PowerSupply_SetOutputCurrent ( hInstrumentHandle , 1 , 5.4 );
	
	PowerSupply_SetOutputState ( hInstrumentHandle , 1 , 1 ); 
	PowerSupply_SetOutputState ( hInstrumentHandle , 1 , 0 );
