...
STD_ERROR   DLLEXPORT	DRV_MultiMeter_Init ( char *pszDriverLocation , char *pszAddressString , int *pHandle )
{
	...
	if ( pHandle )
	{
		...
		do
		{
			...
			if ( !bHandleExists )
				DRIVER_MANAGER_AddConnection( pszAddressString , &(pDriverInfo->InstrumentHandle) , DRIVER_TYPE_MULTI_METER , &(pDriverInfo->InstrumentLockHandle) );
...
Error:
	...
	if ( DRIVER_MANAGER_IsConnectionExistsByType( DRIVER_TYPE_MULTI_METER ) == 0 )
