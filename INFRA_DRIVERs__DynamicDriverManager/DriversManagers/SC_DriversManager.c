	...
	if ( pHandle )
	{
		...
		do
		{
			*pHandle = VariableHandle;
			...
			if ( !bHandleExists )
				DRIVER_MANAGER_AddConnection( pszAddressString , &(pDriverInfo->InstrumentHandle) , DRIVER_TYPE_OSCILLOSCOPE , &(pDriverInfo->InstrumentLockHandle) );
...
Error:
	...
	if ( DRIVER_MANAGER_IsConnectionExistsByType( DRIVER_TYPE_OSCILLOSCOPE ) == 0 )
