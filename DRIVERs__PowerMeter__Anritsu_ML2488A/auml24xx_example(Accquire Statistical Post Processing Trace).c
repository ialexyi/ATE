...
int main (void)
{
	...
	/*- Configure the post processing acquisition ----------------------------------------*/ 
    checkErr(auml24xx_ConfigurePostProcessingAcquisition (auml24xx, "1", VI_TRUE, AUML24XX_VAL_POST_PROCESSING_STATS));

	/*- Configure the statistical post processing ----------------------------------------*/ 
	checkErr(auml24xx_ConfigureStatisticalPostProcessing (auml24xx, "1", AUML24XX_VAL_PROCESSING_TYPE_PDF, -70.0, VI_FALSE,
	...
	/*- Read PMOD trace ----------------------------------------*/ 
    checkErr(auml24xx_ReadStatPostProcessingTrace(auml24xx,"1",&actual_points,trace,400,&initial_x,&increment_x, 10000));

#if defined(_CVI_)	
	WaveformGraphPopup ("Acquired Stat Post Processing Trace", trace, actual_points, VAL_DOUBLE, 1.0, 0.0, initial_x, increment_x);
#endif
