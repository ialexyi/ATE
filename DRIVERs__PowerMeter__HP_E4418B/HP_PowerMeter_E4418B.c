...
void*	DLLEXPORT		Equipment_IsSupported ( int hLowLevelHandle , char *pAddress , char *pID_String , char *pIdentificationText , int *pbSupporting , void **pLocalData )
{
	...
	char							vszSupportdedModels[][32]					=	{{",E441"}};
...
{			
	...
	*InstrumentConnectStatus = 0;
	...
	{
		StdError.error = age441x_init ( szRsrcAddress , VI_ON , VI_OFF , (ViPSession)pInstrumentHandle );
...
void* DLLEXPORT PowerMeter_SetActiveChannel( int hInstrumentHandle , int channel )
{
	STD_ERROR					StdError					=	{0};
...
void* DLLEXPORT PowerMeter_Calibrate( int hInstrumentHandle , int iChannel )
{
	...
	{
		StdError.error = -1;
		strcpy((char*) StdError.pszErrorDescription , "Calibration Fail." );
	}   
