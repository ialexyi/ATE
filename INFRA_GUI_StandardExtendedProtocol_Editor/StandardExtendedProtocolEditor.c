#include <cvirte.h>		
#include <userint.h>
#include "StandardExtendedProtocolEditor.h"
#include "GlobalDefines.h"
#include "toolbox.h"
#include <formatio.h>
#include <ansi_c.h>

#define									MAX_LIST_NUMBER							2048

#define			DEVICE_ID				STANDARD_EXTENDED_PROTOCOL_DRIVER_DEVICE_ID

tsSTD_Extended_CommandItem				gvtListOfCommands[MAX_LIST_NUMBER]	=	{0};
...
int		OpenConfigurationFile( char *pszFilePath )
{
	...
	if ( FileExists( pszFilePath , NULL ))
		if ( strstr( pszFilePath , ".extcnf" ))
		{
			...
			for ( iIndex = 0 ; iIndex < MAX_LIST_NUMBER; iIndex++ )
			{
				iCount = ReadFile ( hFileHandle , gvtListOfCommands[iIndex].max_size , sizeof(tsSTD_Extended_CommandItem) );
...
int main (int argc, char *argv[])
{
	...
	if ((panelHandle = LoadPanel (0, "StandardExtendedProtocolEditor.uir", PANEL)) < 0)
		return -1;
...
void CVICALLBACK clbMenuOpenFile (int menuBar, int menuItem, void *callbackData, int panel)
{
	...
	if ( FileSelectPopup ("", "*.extcnf", "*.extcnf", "Load", VAL_LOAD_BUTTON, 0, 1, 1, 1, szFilePath ))
	{
...
void CVICALLBACK clbMenuSaveFile (int menuBar, int menuItem, void *callbackData, int panel)
{
	...
	if ( FileSelectPopup ("", "*.extcnf", "*.extcnf", "Save", VAL_SAVE_BUTTON, 0, 1, 1, 1, szFilePath ))
	{
...
{
	tsSTD_Extended_CommandItem	tCopyItem		=	{0},
								*pCheckItem		=	NULL,
								*pPilotItem		=	NULL;
	...
	for ( index = 1; index < giNumberOfItems; index++ )
	{
		...
		while( pCheckItem > &(gvtListOfCommands[0]))
		{
			...
			memcpy( &tCopyItem , pCheckItem , sizeof(tsSTD_Extended_CommandItem));
			memcpy( pCheckItem , pPilotItem , sizeof(tsSTD_Extended_CommandItem));
			memcpy( pPilotItem , &tCopyItem , sizeof(tsSTD_Extended_CommandItem));
			pPilotItem = pCheckItem; 
		};
	}
