...
void* DLLEXPORT StandardDevice_GetErrorTextMessage ( int vhInstrumentHandle , int iError , char *pErrorMessage )
{
	...
	FREE_STDERR_COPY_ERR( DAQMX_STD_Driver_GetErrorTextMessage( vhInstrumentHandle , iError , pErrorMessage ));
...
void*	DLLEXPORT		Config_Install_CommentCallback( int *pHandle , pfCommentCallback fFunction , void *pData , int type) 
{
	...
	FREE_STDERR_COPY_ERR( DAQMX_Driver_Config_Install_CommentCallback( pHandle , fFunction , pData , type));
...
void*	DLLEXPORT		Config_Install_ConfigValueCallback( int *pHandle , pfConfigValueCallback fFunction , void *pData , int type) 
{
	...
	FREE_STDERR_COPY_ERR( DAQMX_Driver_Config_Install_ConfigValueCallback( pHandle , fFunction , pData , type));
...
void*	DLLEXPORT		Config_Install_CheckForBreakCallback( int *pHandle , pfCheckForBreakCallback fFunction , void *pData , int type) 
{
	...
	FREE_STDERR_COPY_ERR( DAQMX_Driver_Config_Install_CheckForBreakCallback( pHandle , fFunction , pData , type));
...
void*	DLLEXPORT		Config_LOG_SetAllowState( int iHandle , int bAllowLog ) 
{
	...
	FREE_STDERR_COPY_ERR( DAQMX_Driver_Config_LOG_SetAllowState( iHandle , bAllowLog ));
...
void*	DLLEXPORT		StandardDevice_Init( int iDeviceID , char *pszConnectionName , char *pszAddress , int *pHandle , ... )
{
	...
	FREE_STDERR_COPY_ERR( DAQMX_Driver_Init( pszConnectionName , pszAddress , pHandle )); 
...
void*	DLLEXPORT		StandardDevice_GetConnectionAddress( int hHandle , char **pszAddress )
{
	...
	FREE_STDERR_COPY_ERR( DAQMX_Driver_GetConnectionAddress( hHandle , pszAddress ));
...
void*	DLLEXPORT		StandardDevice_SetValue( int hHandle , char *pCommandName , void *pValue , int iValueLength , int iValueSize , ...  )
{
	...
	FREE_STDERR_COPY_ERR( DAQMX_Driver_SetValue( hHandle , pCommandName , pValue , iValueLength , iValueSize ));
...
void*	DLLEXPORT		StandardDevice_GetValue( int hHandle , char **pGetCommandName , void **pGetValue , int *piValueLength , int *piValueSize , ...  )            
{
	...
	FREE_STDERR_COPY_ERR( DAQMX_Driver_GetValue( hHandle , pGetCommandName , pGetValue , piValueLength , piValueSize ));
...
void*	DLLEXPORT		StandardDevice_GetValue_Ex ( int hHandle , char *pCommandName , void **pGetValue , int *piValueLength , int *piValueSize , ...  )            
{
	...
	FREE_STDERR_COPY_ERR( DAQMX_Driver_GetValue_Ex( hHandle , pCommandName , pGetValue , piValueLength , piValueSize ));
...
void*	DLLEXPORT		StandardDevice_Close( int *pHandle )
{
	...
	FREE_STDERR_COPY_ERR( DAQMX_Driver_Close( pHandle )); 
...
void*	DLLEXPORT		StandardDevice_Unplug( int hHandle , double lfTimeOut )
{
	...
	FREE_STDERR_COPY_ERR( DAQMX_Driver_Unplug( hHandle , lfTimeOut ));
...
void*	DLLEXPORT		StandardDevice_PlugIn( int hHandle , double lfTimeOut )
{
	...
	FREE_STDERR_COPY_ERR( DAQMX_Driver_PlugIn( hHandle , lfTimeOut ));
...
void*	DLLEXPORT		StandardDevice_EnablePlugInAutomatically( int hHandle , int bEnable )
{
	...
	FREE_STDERR_COPY_ERR( DAQMX_Driver_EnablePlugInAutomatically( hHandle , bEnable ));
...
void*	DLLEXPORT		StandardDevice_InstallReceiveCallbackFunction( int hHandle , void *pfFunction , void *pData )  
{
	...
	FREE_STDERR_COPY_ERR( DAQMX_Driver_InstallReceiveCallbackFunction( hHandle , pfFunction , pData ));
...
void*	DLLEXPORT		StandardDevice_LoadConfigFile( int hHandle , char *pFileName )
{
	...
	FREE_STDERR_COPY_ERR( DAQMX_Driver_LoadConfigFile( hHandle , pFileName ));
...
void*	DLLEXPORT		StandardDevice_LoadConfigBuffer( int hHandle , char *pBuffer , int iBufferSize )
{
	...
	FREE_STDERR_COPY_ERR( DAQMX_Driver_LoadConfigBuffer( hHandle , pBuffer , iBufferSize ));
...
void*	DLLEXPORT		StandardDevice_Check_Connection( int hHandle , char *pCommandName , int *piStatus )
{
	...
	FREE_STDERR_COPY_ERR( DAQMX_Driver_Check_Connection( hHandle , pCommandName , piStatus ));
...
void*	DLLEXPORT		StandardDevice_Get_Commands_List( int hHandle , char ***pCommandsList , int *piNumberOfCommands )     
{
	...
	FREE_STDERR_COPY_ERR( DAQMX_Driver_Get_Commands_List( hHandle , pCommandsList , piNumberOfCommands ));
...
//	char				*pGetCommandName	=	0;
	...
	StandardDevice_Init( DEVICE_ID , "MBF" , "" , &hHandle ) ;
	
	StandardDevice_LoadConfigFile( hHandle , "C:\\emi\\WorkSpace\\MBF_Config.daqcnf" ); 
	
	iValue = 0;
	StandardDevice_SetValue( hHandle , "ENABLE" , &iValue , 1 , sizeof(iValue) ); 
	...
	iValue = 0;
	StandardDevice_SetValue( hHandle , "FILTER_SELECT" , &iValue , 1 , sizeof(iValue) ); 
